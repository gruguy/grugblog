<template>
  <div class="min-h-screen bg-background">
    ...
    <nav class="sticky top-0 z-50 bg-card border-b border-border">
      <div class="container mx-auto px-4">
        <div class="flex items-center justify-between h-16">
          <!-- Logo -->
          <RouterLink to="/" class="text-2xl font-bold text-primary">个人博客</RouterLink>
          ...
          <div class="hidden md:flex items-center space-x-4">
            ...
            <div class="flex items-center space-x-2">
              ...
              <template v-else>
                <!-- 消息图标 -->
                <button class="p-2 rounded-full hover:bg-muted transition-colors relative" title="消息">
                ...
                <div class="relative group">
                  <button class="flex items-center gap-2">
                    <div v-if="!userStore.user?.avatar" class="w-8 h-8 rounded-full bg-gradient-to-br from-primary to-secondary flex items-center justify-center text-white font-medium text-xs border border-border">
                  ...
                  <!-- 下拉菜单 -->
                  <div class="absolute right-0 mt-2 w-48 bg-card border border-border rounded-md shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                    <div class="py-2">
                      <RouterLink
                        to="/profile"
      ...
      <!-- 移动端菜单 -->
      <div v-if="showMobileMenu" class="md:hidden border-t border-border bg-card">
    ...
    <!-- 登录弹窗 -->
    <Modal
      v-model:modelValue="showLoginModal"
      title="用户登录"
    >
      <div class="p-0">
    ...
    <!-- 注册弹窗 -->
    <Modal
      v-model:modelValue="showRegisterModal"
      title="用户注册"
    >
      <div class="p-0">
        <form @submit.prevent="handleRegister">
          <div class="mb-4 px-6 pt-6">
            <label for="regUsername" class="block text-sm font-medium mb-1">用户名</label>
            <input
              type="text"
              id="regUsername"
          ...
          <div class="mb-4 px-6">
            <label for="regEmail" class="block text-sm font-medium mb-1">邮箱</label>
            <input
              type="email"
              id="regEmail"
          ...
          <div class="mb-4 px-6">
            <label for="regPassword" class="block text-sm font-medium mb-1">密码</label>
            <input
              type="password"
              id="regPassword"
          ...
          <div class="mb-6 px-6">
            <label for="regConfirmPassword" class="block text-sm font-medium mb-1">确认密码</label>
            <input
              type="password"
          ...
          <div v-if="registerForm.password !== registerForm.confirmPassword" class="mb-4 px-6 text-red-500 text-sm">
            两次输入的密码不一致
          </div>
          <div class="mb-6 px-6">
            <button
...
const handleLogin = async () => {
  ...
  try {
    await userStore.userLogin(loginForm.value.username, loginForm.value.password);
    showLoginModal.value = false;
